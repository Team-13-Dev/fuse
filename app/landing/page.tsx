...
import { ArrowUpRight, Target, Activity, Zap, Check } from 'lucide-react'; // Removed unused `Play` import
...
const Navbar = () => {
  ...
  return (
    <nav className={` fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? ' backdrop-blur-xl bg-white  text-black py-1 shadow-md' : 'py-1.5 text-white'}`}>
          <div className="px-6 h-[68px] flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center gap-6 text-sm">
              ...
              <button className='flex items-center gap-1 shadow-md text-sm px-4 py-1 text-black  border-[#E1E1E2] border bg-white rounded-md'>
                Sign Up
                <ChevronRight size={10}/>
              </button>
              {/* <SignedIn>
                <Link href={"/dashboard"} className={`text-white  border-none ${isScrolled ? "bg-[#2F46F8]" : "bg-[#0F1012] "}  border  px-8 py-2.5 rounded-full font-semibold text-sm hover:bg-[#212222] hover:text-white cursor-pointer transition-all duration-300`}>
                  Dashboard
                </Link>
                <UserButton />
              </SignedIn>
              <SignedOut>
                <Link href={"/sign-in"} className="text-sm font-normal text-gray-300 hover:text-white transition">Sign in</Link>
                <Link href={"/sign-up"} className="bg-[#0F1012] text-white border border-[#36373B] px-8 py-2.5 rounded-[10px] font-semibold text-sm hover:bg-[#00FFA3] hover:text-black cursor-pointer hover:shadow-[0_0_20px_rgba(0,255,163,0.4)] transition-all duration-300">
                  Sign Up
                </Link>
              </SignedOut> */}
            </div>
          ...
          {isMenuOpen && (
            <div className="md:hidden absolute w-full bg-white border-b p-6 flex flex-col gap-4">
              ...
              <button className="w-full bg-[#2F46F8] text-white py-3 rounded-lg font-bold">Sign Up</button>
            </div>
          )}
    </nav>
  );
...
const Hero = () => {
  ...
  return (
    <section ref={containerRef} className="relative h-[100dvh] w-full overflow-hidden bg-[#111111] flex items-end pb-24 md:pb-32">
      ...
      <div className="relative z-10 w-full max-w-7xl px-6 mx-auto md:px-12">
        <div className="max-w-4xl text-[#F5F3EE]">
          ...
          <div className="flex flex-col items-start gap-6 hero-elem md:flex-row md:items-center">
            <button className="magnetic-hover cursor-pointer bg-[#2F47F2] text-white px-8 py-4 rounded-full font-bold text-lg uppercase flex items-center gap-2 group">
    ...
    <div className="relative h-48 w-full" style={{ perspective: '1000px' }}>
      {cards.map((card, i) => {
        ...
        return (
          <div 
            key={i}
            className="absolute top-0 left-0 w-full p-4 border rounded-[2rem] bg-[#f5f5f5] border-[#111111]/10 flex items-center justify-between transition-all duration-700 shadow-sm"
            style={{
...
const TelemetryTypewriter = () => {
  ...
  return (
    <div className="w-full h-48 p-6 bg-[#111111] text-[#E8E4DD] rounded-[2rem] font-data text-xs overflow-hidden relative shadow-inner">
...
const ProtocolScheduler = () => {
  ...
  return (
    <div ref={containerRef} className="relative w-full h-48 p-6 bg-[#E8E4DD] border border-[#111111]/10 rounded-[2rem] overflow-hidden flex flex-col justify-between">
    ...
    <section id="protocol" className="py-32 bg-[#111111] text-white">
      ...
      <div ref={mainRef} className="relative max-w-5xl mx-auto px-6">
        {steps.map((step, i) => (
          <div
            key={i}
            className="protocol-card w-full min-h-[50vh] mx-auto rounded-[3rem] bg-[#fff] text-[#1d1d1d] border border-[#111111]/10 flex flex-col md:flex-row items-center justify-center p-8 md:p-16 mb-16 relative shadow-lg will-change-transform"
